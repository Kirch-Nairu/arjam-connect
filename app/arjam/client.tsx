...
const platformShort: Record<Platform, string> = { facebook: "FB", instagram: "IG", tiktok: "TT" };
const statusLabel: Record<ConversationStatus, string> = { new: "New", qualifying: "Qualifying", qualified: "Qualified", needs_human: "Needs human", follow_up: "Follow up", closed: "Closed" };
...
export default function ArjamDashboard() {
  ...
  useEffect(() => {
    ...
    void readState().then(setState).catch(() => undefined);
  ...
  const filtered = useMemo(() => {
    ...
    return state.conversations.filter((conversation) => {
      ...
      return [conversation.customerName, conversation.customerHandle, platformLabel[conversation.platform], conversation.inquiry.destination, conversation.messages.at(-1)?.text]
  ...
  async function openConversation(id: string) {
    setActiveId(id);
    try { setState(await markConversationRead(id)); } catch {}
  }
  ...
  async function sendAgentReply() {
    ...
    setDraft("");
    try { setState(await sendAgentMessage(active.id, text)); } catch { setDraft(text); }
  }
  ...
  return (
    <main className="workspace-shell">
      <aside className="workspace-nav">
        <div>
          <div className="workspace-brand"><div className="logo-mark small">A</div><div><strong>Arjam Connect</strong><span>Travel & Tours</span></div></div>
          <nav>
        ...
        <div className="workspace-nav-bottom"><Link href="/demo?auto=1" target="_blank">Open demo customer</Link><button onClick={logout}>Sign out</button></div>
      </aside>

      <section className="workspace-main">
        <header className="workspace-header"><div><h1>{view === "inbox" ? "Inbox" : view === "dashboard" ? "Dashboard" : view === "customers" ? "Customers" : "FAQ responses"}</h1>{view === "inbox" && <p>All customer conversations in one place.</p>}</div><Link className="btn btn-secondary" href="/demo?auto=1" target="_blank">Open tester</Link></header>

        {view === "inbox" && <InboxView conversations={filtered} active={active} activeId={activeId} search={search} filter={filter} draft={draft} onSearch={setSearch} onFilter={setFilter} onSelect={(id) => void openConversation(id)} onDraft={setDraft} onSend={() => void sendAgentReply()} onTakeover={(id, takeover) => void setAgentTakeover(id, takeover).then(setState)} onStatus={(id, status) => void setConversationStatus(id, status).then(setState)} />}
        {view === "dashboard" && <DashboardView conversations={state.conversations} metrics={metrics} onOpen={(id) => { void openConversation(id); setView("inbox"); }} />}
        {view === "customers" && <CustomersView conversations={state.conversations} onOpen={(id) => { void openConversation(id); setView("inbox"); }} />}
        {view === "faq" && <FaqView items={state.faqItems} onUpdate={(id, patch) => void updateFaq(id, patch).then(setState)} />}
      </section>
    </main>
  );
}
...
function NavItem({ active, label, count, onClick }: { active: boolean; label: string; count?: number; onClick: () => void }) {
  return <button className={active ? "nav-item active" : "nav-item"} onClick={onClick}><span>{label}</span>{Boolean(count) && <b>{count}</b>}</button>;
}
...
function DashboardView({ conversations, metrics, onOpen }: { conversations: Conversation[]; metrics: { open: number; human: number; qualified: number }; onOpen: (id: string) => void }) {
  return <div className="page-body"><div className="summary-grid"><SummaryCard label="Open inquiries" value={metrics.open} /><SummaryCard label="Needs human reply" value={metrics.human} attention={metrics.human > 0} /><SummaryCard label="Qualified" value={metrics.qualified} /></div><section className="panel simple-panel"><div className="panel-title"><h2>Recent inquiries</h2><span>{conversations.length} total</span></div><div className="recent-list">{conversations.slice(0, 7).map((conversation) => <button key={conversation.id} onClick={() => onOpen(conversation.id)}><span className={`platform-chip ${conversation.platform}`}>{platformShort[conversation.platform]}</span><span className="recent-main"><b>{conversation.customerName}</b><small>{conversation.messages.at(-1)?.text}</small></span><span className={`status-pill ${conversation.status}`}>{statusLabel[conversation.status]}</span><time>{formatTime(conversation.updatedAt)}</time></button>)}</div></section></div>;
}
...
function Detail({ label, value }: { label: string; value: string }) { return <div className="detail-row-clean"><span>{label}</span><b>{value}</b></div>; }
function CustomersView({ conversations, onOpen }: { conversations: Conversation[]; onOpen: (id: string) => void }) { return <div className="page-body"><section className="panel table-panel"><table><thead><tr><th>Customer</th><th>Channel</th><th>Destination</th><th>Status</th><th>Last activity</th><th /></tr></thead><tbody>{conversations.map((conversation) => <tr key={conversation.id}><td><b>{conversation.customerName}</b><small>{conversation.customerHandle}</small></td><td>{platformLabel[conversation.platform]}</td><td>{conversation.inquiry.destination ?? "—"}</td><td><span className={`status-pill ${conversation.status}`}>{statusLabel[conversation.status]}</span></td><td>{formatTime(conversation.updatedAt)}</td><td><button className="text-link" onClick={() => onOpen(conversation.id)}>Open</button></td></tr>)}</tbody></table></section></div>; }
function FaqView({ items, onUpdate }: { items: FaqItem[]; onUpdate: (id: string, patch: Partial<FaqItem>) => void }) { return <div className="page-body faq-page-clean"><div className="faq-intro"><h2>Chatbot answers</h2><p>These are the approved responses used for common customer questions. Edit the wording here without changing the chatbot code.</p></div><div className="faq-list-clean">{items.map((item) => <FaqEditor key={item.id} item={item} onUpdate={onUpdate} />)}</div></div>; }
function FaqEditor({ item, onUpdate }: { item: FaqItem; onUpdate: (id: string, patch: Partial<FaqItem>) => void }) { const [answer, setAnswer] = useState(item.answer); useEffect(() => setAnswer(item.answer), [item.answer]); return <details className="faq-item-clean"><summary><span><b>{item.title}</b><small>{item.category}</small></span><span className={item.enabled ? "faq-enabled" : "faq-disabled"}>{item.enabled ? "Enabled" : "Disabled"}</span></summary><div className="faq-editor-clean"><label>Response<textarea rows={5} value={answer} onChange={(event) => setAnswer(event.target.value)} /></label><div><label className="toggle-line"><input type="checkbox" checked={item.enabled} onChange={(event) => onUpdate(item.id, { enabled: event.target.checked })} /> Enable this response</label><button className="btn btn-primary" onClick={() => onUpdate(item.id, { answer })}>Save response</button></div></div></details>; }
